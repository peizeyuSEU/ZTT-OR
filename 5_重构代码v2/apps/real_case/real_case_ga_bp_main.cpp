...
Loaded loadCase(const std::string& dir, double intracity, double price, double quota, double delta, bool explicitCost) {
    ...
    Instance& x=L.inst; x.transportCostMode = explicitCost ? TransportCostMode::EXPLICIT_ARC_COST : TransportCostMode::FROZEN_PIECEWISE; x.numDC=(int)L.dc.size(); x.numRetailer=(int)L.market.size(); x.supplierX=num(bynode.at("S1"),"longitude"); x.supplierY=num(bynode.at("S1"),"latitude");
    x.dcX.resize(x.numDC);x.dcY.resize(x.numDC);x.retX.resize(x.numRetailer);x.retY.resize(x.numRetailer);x.f.resize(x.numDC);x.F.resize(x.numDC);x.g.resize(x.numDC);x.L.resize(x.numDC);x.fc.resize(x.numDC);x.a_dist.resize(x.numDC);x.b.resize(x.numDC);x.beta.resize(x.numDC);x.w.assign(x.numDC,0.0);x.mu.resize(x.numRetailer);x.variance.resize(x.numRetailer);x.reservePrice.resize(x.numRetailer);x.dist.assign(x.numRetailer,std::vector<double>(x.numDC));x.bb.assign(x.numRetailer,std::vector<double>(x.numDC));x.supplierDcTransportCostPerTonne.assign(x.numDC,0.0);x.dcMarketTransportCostPerTonne.assign(x.numRetailer,std::vector<double>(x.numDC,0.0));
    ...
    for(int j=0;j<x.numDC;++j){x.supplierDcTransportCostPerTonne[j]=num(sm.at(L.dc[j]),"transport_cost_baseline");for(int i=0;i<x.numRetailer;++i){auto&r=dm.at(L.dc[j]+"|"+L.market[i]);double d=num(r,"road_distance_km");if(L.dc[j]==L.market[i])d=intracity;x.dist[i][j]=d;x.bb[i][j]=x.k*d;x.dcMarketTransportCostPerTonne[i][j]=num(r,"transport_cost_baseline");if(L.dc[j]==L.market[i])x.dcMarketTransportCostPerTonne[i][j]=0.39*d;L.arcs.push_back({L.dc[j],L.market[i]});}x.b[j]=x.k*x.a_dist[j];}
    return L;
}
void writeDry(const std::string& out,const Loaded&L,double intracity,double price,double quota,int seed){
    ...
    std::ofstream c(out+"/resolved_real_case_instance.csv");c<<"dc_index,dc_id,market_index,market_id,distance_km,transport_cost_coefficient,transport_emission_coefficient\n";for(int j=0;j<L.inst.numDC;++j)for(int i=0;i<L.inst.numRetailer;++i)c<<j<<","<<L.dc[j]<<","<<i<<","<<L.market[i]<<","<<L.inst.dist[i][j]<<","<<L.inst.dcMarketTransportCostPerTonne[i][j]<<","<<L.inst.k*L.inst.dist[i][j]<<"\n";
