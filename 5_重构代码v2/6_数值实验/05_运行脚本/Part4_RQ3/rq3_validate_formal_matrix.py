...
manifest=(next((root/'results/paper_experiments/part4_carbon_policy/RQ3_price_cap').glob('screening_42runs_resumable_*'))/'SCREENING_FINALIZATION_MANIFEST.txt').read_text()
if manifest.count('formal_stage_status:')!=1 or 'formal_stage_status: FINAL_FROZEN_PENDING_EXECUTION_APPROVAL' not in manifest:e.append('manifest formal status')
if manifest.count('raw_solver_outputs_modified:')!=1 or 'raw_solver_outputs_modified: false' not in manifest:e.append('manifest raw flag')
for z in ['formal_matrix_status: FINAL_FROZEN_PENDING_EXECUTION_APPROVAL','formal_solver_started: false','screening_solver_rerun_performed: false','screening_status: FINAL_COMPLETE_AND_FROZEN','completed_runs: 42']:
 if z not in manifest:e.append('manifest '+z)
